...
import { prepareWithSegments, layoutNextLineRange, materializeLineRange, type PreparedTextWithSegments } from "@chenglou/pretext";
  ...
  useEffect(() => {
    ...
    const reflow = () => {
      try {
        ...
        for (let i = 0; i < 50; i++) { // Safety cap for lines
          if (currentCursor.segmentIndex < 0 || currentCursor.segmentIndex >= prepared.segments.length) break;
          ...
          const line = layoutNextLineRange(prepared, currentCursor, Math.max(1, effectiveWidth));
          
          if (!line) {
            break;
          }
          
          if (line.start.segmentIndex === line.end.segmentIndex && line.start.graphemeIndex === line.end.graphemeIndex) break;
