...
export default function WaveGradientBar({ colors, topic, className = "" }: WaveGradientBarProps) {
  ...
  const colorBalance = 0.0;
  const warpStrength = 1.0;
  const warpFrequency = 5.0;
  const warpSpeed = 2.0;
  const warpAmplitude = 50.0;
  const blendAngle = 0.0;
  const blendSoftness = 0.05;
  const rotationAmount = 500.0;
  const noiseScale = 2.0;
  const grainAmount = 0.1;
  const grainScale = 2.0;
  const grainAnimated = false;
  const contrast = 1.5;
  const gamma = 1.0;
