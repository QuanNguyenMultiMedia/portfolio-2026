    ...
    >
      ...
      <div className="fixed top-8 left-8 md:top-12 md:left-12 z-50 pointer-events-auto flex items-center gap-12">
        ...
        <div className="hidden md:flex flex-col gap-0.5">
          <span className="text-[10px] font-mono tracking-[0.4em] uppercase opacity-60">{project.title}</span>
        </div>
      </div>
      ...
      <div className="lenis-content h-screen w-fit flex items-stretch">
          ...
          <div className="relative z-10">
            <div className="flex flex-col gap-8 md:gap-16">
              <div className="space-y-4">
                ...
                >
                  <span className="text-[10px] font-mono tracking-[0.5em] uppercase">{project.category}</span>
                  <div className="h-px w-8 bg-foreground/20" />
                  <span className="text-[10px] font-mono tracking-[0.5em] uppercase">{project.year}</span>
                </motion.div>
                ...
                >
                  {project.title.split(' ').map((word, i) => (
                    <span key={i} className={i % 2 === 1 ? "italic font-light block ml-12 md:ml-32 text-primary" : "block"}>
                      {word}
                    </span>
                  ))}
                </motion.h1>
          ...
          >
            ...
            {screen.type === "bento" && (
              <div className="w-full h-full py-24 md:py-32 grid grid-cols-12 gap-6 md:gap-12 items-center">
                ...
                >
                  <img src={screen.images?.[0]} className="w-full h-full object-cover transition-transform duration-[2000ms] group-hover:scale-110" />
                ...
                <div className="col-span-4 space-y-12">
                   ...
                   >
                     <img src={screen.images?.[1]} className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" />
                   </motion.div>
                   <motion.div 
                     initial={{ opacity: 0, x: 20 }}
                     whileInView={{ opacity: 1, x: 0 }}
                   ...
                   >
                     <img src={screen.images?.[2]} className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" />
                   </motion.div>
                </div>
              </div>
            )}
        ...
        <section className="flex-shrink-0 w-[100vw] md:w-[80vw] relative flex flex-col justify-center px-12 md:px-24 overflow-hidden group">
          ...
          >
            <div className="space-y-4">
               <span className="text-[10px] font-mono tracking-[0.8em] uppercase opacity-40 group-hover:text-primary transition-colors">Want to see more?</span>
            ...
            <div className="relative w-full aspect-video md:aspect-[21/9] overflow-hidden shadow-2xl">
               <motion.img 
                 src={projects[(projects.findIndex(p => p.slug === slug) + 1) % projects.length].coverImage}
            ...
            <div className="flex justify-between items-center opacity-30 group-hover:opacity-100 transition-opacity">
               <span className="text-[10px] font-mono tracking-widest uppercase">Click to advance</span>
               <div className="h-px flex-1 mx-8 bg-foreground/20" />
               <span className="text-[10px] font-mono tracking-widest uppercase">/ 2026</span>
            </div>
          </Link>
        </section>
      </div>
